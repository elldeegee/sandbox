...
# dates
date_born <- rep_len(as.character(sample(seq(as.Date('1950/01/01'), as.Date('1999/12/31'), by="day"),
                                         replace = TRUE)),
                     length.out = nrow(baby_names))

date_signed_up <- rep_len(as.character(sample(seq(as.Date('2011/01/01'), as.Date('2014/12/31'), by="day"),
                                              replace = TRUE)),
                          length.out = nrow(baby_names))
...
purchases <- cbind(purchases_tbl,data_frame(n_scoops,flavor),days_between) %>%
  mutate(date_purchased = date_signed_up + days_between,
         date_purchased = if_else(date_purchased > as.Date("2014-12-31"),as.Date("2014-12-31"),date_purchased)) %>%
